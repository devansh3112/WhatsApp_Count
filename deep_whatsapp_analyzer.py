        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            for message in messages:
                ...
                if match:
                    ...
                    # Parse date and time
                    try:
                        date_obj = datetime.strptime(f"{date_str}, {time_str}", "%d/%m/%y, %I:%M %p")
                        ...
                        if text:
                            ...
                            if "<Media omitted>" in text or "image omitted" in text or "video omitted" in text:
                                ...
                                message_lengths[sender.strip()].append(len(words))
...
def plot_message_count(data, output_dir):
    sorted_counts = sorted(data['message_count'].items(), key=lambda x: x[1], reverse=True)
...
def plot_media_count(data, output_dir):
    sorted_counts = sorted(data['media_count'].items(), key=lambda x: x[1], reverse=True)
...
def plot_activity_over_time(data, output_dir):
    ...
    plt.plot(date_objects, counts, marker='o', linestyle='-', color='royalblue')
    plt.xlabel('Date')
    plt.ylabel('Number of Messages')
...
def plot_average_message_length(data, output_dir):
    sorted_lengths = sorted(data['avg_message_length'].items(), key=lambda x: x[1], reverse=True)
...
def generate_statistics_report(data, output_dir):
    with open(f'{output_dir}/statistics_report.txt', 'w', encoding='utf-8') as f:
        ...
        sorted_members = sorted(data['message_count'].items(), key=lambda x: x[1], reverse=True)[:5]
        ...
        if total_media > 0:
            ...
            f.write("\nTop 3 Media Senders:\n")
            sorted_media = sorted(data['media_count'].items(), key=lambda x: x[1], reverse=True)[:3]
            for i, (member, count) in enumerate(sorted_media, 1):
                percentage = (count / total_media) * 100
                f.write(f"{i}. {member}: {count} media messages ({percentage:.1f}%)\n")
        
        f.write("\n")
        
        # Activity patterns
        most_active_hour = max(data['hourly_activity'].items(), key=lambda x: x[1])[0]
        most_active_day = max(data['weekday_activity'].items(), key=lambda x: x[1])[0]
        
        f.write(f"Most Active Hour: {most_active_hour}:00 - {most_active_hour+1}:00\n")
        ...
        if data['avg_message_length']:
            ...
            f.write(f"Overall Average Message Length: {overall_avg:.1f} words\n")
            f.write(f"Member with Longest Messages: {max_avg[0]} ({max_avg[1]:.1f} words on average)\n\n")
        ...
        if data['word_count']:
            ...
            filtered_words = {word: count for word, count in data['word_count'].items() if len(word) > 2}
        ...
        if data['emoji_count']:
            ...
            top_emojis = sorted(data['emoji_count'].items(), key=lambda x: x[1], reverse=True)[:5]
